...
readdirSync("./redirek").forEach(file => {
    ...
    let lrp = btprs(file)
    console.log(lrp)
    if (!lrp) {rmSync("./redirek/" + file, { recursive: true, force: true })}
    else if (lrp){
        try {
    readdirSync("./redirek/" + file).forEach(file2 => {
        if (file2 !== "creds.json") {
            unlinkSync("./redirek/" + file + "/" + file2)
        } 
    })
    } catch {}}})
...
readdirSync("./MysticSession").forEach(file => {
    if (file !== 'creds.json') {
        unlinkSync("./sesi/" + file, { recursive: true, force: true })}})    
...
if (connection == 'open') {
console.log(chalk.yellow('Scan Qr untuk Menghubungkan Bot'))}
}
...
conn.welcome = '*@user Telah Bergabung Di Grup @subject*'
conn.bye = '*Selamat Tinggal @user'
conn.spromote = '*@user Sekarang Pangkat Mu Sebagai Admin !!*'
conn.sdemote = '*@user Pangkat Admin Mun Telah Di Cabut !!*'
conn.sDesc = '*Deskripsi Di Ubah Menjadi:\n\n* @desc'
conn.sSubject = '*Nama Grup Telah Di Ubah Menjadi\n\n* @subject'
conn.sIcon = '*Foto Grup telah Di Ubah*'
conn.sRevoke = '*Link Group Telah Di Ubah* @revoke'
...
console.log(chalk.cyanBright(`📋Autocleartmp`))
}, 180000)
_quickTest()
.then(() => conn.logger.info(`Memuat...\n`))
.catch(console.error)
